...
class UserControllerTest extends BaseControllerTest {
    ...
    void me_ShouldReturnUser() throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(get("/api/v2/user/me"))
    ...
    void update_ShouldReturnAccepted() throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(put("/api/v2/user/update")
    ...
    void user_shouldReturnOtherUserDTO() throws Exception {
        ...
        OtherUserDTO otherUserDTO = OtherUserDTO.builder()
                .username(username)
                .role(Role.User)
                .image("https://radush.ro/assets/images/radush.png")
                .build();

        when(userService.info(otherUserDTO.getUsername())).thenReturn(CompletableFuture.completedFuture(otherUserDTO));


        MvcResult mvcResult = mvc.perform(get("/api/v2/user/user/{username}", username))
                .andExpect(request().asyncStarted())
                .andReturn();
    ...
    void user_ShouldReturnBadRequest_WhenUserNotFound() throws Exception {
        when(userService.me()).thenReturn(
                CompletableFuture.failedFuture(new InvalidFieldException("User not found."))
        );
