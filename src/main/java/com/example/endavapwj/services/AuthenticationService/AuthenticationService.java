...
public interface AuthenticationService {
   ...
   *
   * <p>Checks whether an user already exists.
   *
   * <p>Creates a new user entry with an encoded password and generates an email validation token.
   *
   ...
   * as verified. Removes the consumed validation token.
   *
  ...
  /**
   * Attempts to authenticate a user. Verifies that the user exists, checks for account lockout, and
   * validates the password. Generates both access and refresh JWT tokens on successful
   * authentication.
   *
   ...
   * @throws AccountLockedException if the account is temporarily locked due to repeated failed
   *     attempts
   */
  @Transactional
