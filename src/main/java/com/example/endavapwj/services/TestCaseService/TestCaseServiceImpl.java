...
public class TestCaseServiceImpl implements TestCaseService {
    ...
    public TestCaseServiceImpl(TestCaseRepository testCaseRepository, UserRepository userRepository, ProblemRepository problemRepository, JwtUtil jwtUtil) {
    ...
    public CompletableFuture<Map<String,String>> addTestCase(CreateTestCaseDTO createTestCaseDTO){
        User u = userRepository.findByUsername(jwtUtil.extractUsername()).orElseThrow(()->new NotFoundException("User not found"));
        Problem p = problemRepository.findByTitle(createTestCaseDTO.getProblemTitle()).orElse(problemRepository.findById(createTestCaseDTO.getProblemId()).orElseThrow(()->new NotFoundException("Problem not found")));
