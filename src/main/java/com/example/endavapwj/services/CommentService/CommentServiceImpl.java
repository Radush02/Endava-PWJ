...
public class CommentServiceImpl implements CommentService {
    ...
    public CompletableFuture<Map<String, String>> addComment(AddCommentDTO commentDTO) {
        ...
        Comment c = Comment.builder()
                ...
                .user(u)
                .problem(problemRepository.findById(commentDTO.getProblemId()).orElseThrow(()->new NotFoundException("Problem not found")))
                .build();
    ...
    public CompletableFuture<Map<String,String>> addVote(AddCommentVoteDTO voteDTO){
        User u = userRepository.findByUsernameIgnoreCase(jwtUtil.extractUsername()).orElseThrow(()->new NotFoundException("User not found"));
        Comment c = commentRepository.findById(voteDTO.getCommentId()).orElseThrow(()->new NotFoundException("Comment not found"));
        try{
            CommentVote cv = CommentVote.builder()
                    .comment(c)
                    .user(u)
