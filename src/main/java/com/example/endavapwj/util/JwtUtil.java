 ...
 *
 * <p>Access tokens are issued with a shorter validity period and are used for authorizing requests.
 * Refresh tokens are issued with a longer validity period and are used for obtaining new access
 * tokens without requiring the user to log in again.
 *
 * <p>Both token types include the username as the subject claim and are signed using an HMAC
 * SHA-256 key derived from the configured secret.
 */
@Service
public class JwtUtil {
  ...
  /**
   * Generates a new access token for the given username. The token is valid for 12 hours from the
   * time of issuance.
   *
  ...
  /**
   * Generates a new refresh token for the given username. The token is valid for 30 days.
   *
